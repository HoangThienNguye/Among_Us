...
# Pas dit percentage aan: hoe sterk het op een Among Us moet lijken
...
def knip_amongus(image):
    ...
    return gray[y:y + h, x:x + w], mask[y:y + h, x:x + w]
...
def kleur_maskers(hsv, soepel=True):
    """Hue blijft herkenbaar bij weinig of veel licht; saturatie mag lager."""
...
def herken_kleur(roi):
    """Bepaal rood/blauw. Paarsblauw telt als blauw, niet als rood."""
    if roi.size == 0:
        return None
